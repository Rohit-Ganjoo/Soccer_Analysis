...
WITH performance AS (
    SELECT 
        ...
        ROUND(AVG(pa.overall_rating) OVER(PARTITION BY p.player_name, YEAR(pa.date) ORDER BY p.player_name, YEAR(pa.date)), 0) AS Performance_Rating 
    FROM 
        player AS p
...
SELECT 
    ...
    ROUND(AVG(Dribbling), 2) AS Average_Dribbling,
    ROUND(AVG(Curve), 2) AS Average_Curve,
    ROUND(AVG(Free_Kick_Accuracy), 2) AS Average_Free_Kick_Accuracy,
    ROUND(AVG(Long_Pass), 2) AS Average_Long_Pass,
    ROUND(AVG(Ball_Control), 2) AS Average_Ball_Control,
    ROUND(AVG(Acceleration), 2) AS Average_Acceleration,
    ROUND(AVG(Sprint_Speed), 2) AS Average_Sprint_Speed,
    ROUND(AVG(Agility), 2) AS Average_Agility,
    ROUND(AVG(Reactions), 2) AS Average_Reactions,
    ROUND(AVG(Balance), 2) AS Average_Balance,
    ROUND(AVG(Shot_Power), 2) AS Average_Shot_Power,
    ROUND(AVG(Jumping), 2) AS Average_Jumping,
    ROUND(AVG(Stamina), 2) AS Average_Stamina,
    ROUND(AVG(Strength), 2) AS Average_Strength,
    ROUND(AVG(Long_Shot), 2) AS Average_Long_Shot
FROM 
    Player_age 
GROUP BY 
    1
